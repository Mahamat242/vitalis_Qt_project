    ...
    : QWidget(parent)
{
    ...
    QLabel *prescTitle = new QLabel("<h3>Ordonnance ou médicaments prescrits</h3>", this);
    ...
    prescriptionTable->setHorizontalHeaderLabels({"code", "Médicament", "Nombre de jours"});
    ...
    QPushButton *removeMedocButton = new QPushButton("Retirer le médicament sélectionné");
    ...
    // connexions
    connect(addMedocButton, &QPushButton::clicked, this, &consultationForm::addMedicamentToPrescription);
    connect(removeMedocButton, &QPushButton::clicked, this, &consultationForm::removeMedoc);
    connect(save, &QPushButton::clicked, this, &consultationForm::saveConsultation);
...
void consultationForm::loadMedecin()
{
    ...
    while(query.next())
    {
        ...
        medecin->addItem(QString("Dr. %1 (Matricule : %2)").arg(nom).arg(matricule), matricule);
    }
}
...
void consultationForm::loadPatient()
{
    patient->clear();
    QSqlQuery query;
    query.prepare("select select num_ss, nom from Patient order by nom");
    ...
    while(query.next())
    {
        ...
        QString nom = query.value(1).toString();
        // on stocke le matricule comme QVariant
        medecin->addItem(QString("%1 (N° SS : %2)").arg(nom).arg(num_ss), num_ss);
    }
}
...
void consultationForm::loadMedicament()
{
    ...
    while(query.next())
    {
        ...
        // on stocke le matricule comme QVariant
        medecin->addItem(QString("%1 (%2)").arg(libelle, code), code);
    }
}
...
void consultationForm::addMedicamentToPrescription()
{
    ...
    for (int i = 0; i < prescriptionTable->rowCount(); ++i)
    {
        ...
        {
            QMessageBox::warning(this, "Doublon", "Ce médicament est déjà présent dans l'ordonnance");
            return;
        }
    }
    ...
    prescriptionTable->setItem(row, 2, new QTableWidgetItem(QString::number(nb_jours)));
}
...
void consultationForm::removeMedoc()
{
    ...
    {
        QMessageBox::warning(this, "Sélection requise", "Veuillez sélectionner une ligne à retirer.");
    }
}
...
void consultationForm::saveConsultation()
{
    ...
    {
        QMessageBox::warning(this, "Incomplet", "un médecin et un patient doivent être sélectionner.");
        return;
    }
    ...
    QSqlQuery query;
    query.prepare("insert into Consultation (date, medecin_matricule, patient_num_ss) values (:date, :medecin, :patient)");
    ...
    prescQuery.prepare("insert into Prescrit (medicament_code, consultation_num, nombre_jours) values (:code_medoc, :consult_num, :nb_jours)");
    ...
    db.commit();
    QMessageBox::information(this, "Succèes", QString("Consultaiton N°%1 enregistrée avec succès !").arg(numeroConsultation));
